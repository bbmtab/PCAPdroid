...
static void check_domain_block_rules(pcapdroid_t *pd, pd_conn_t *data, const zdtun_5tuple_t *tuple) {
    if(data->info && data->info[0]) {
        ...
        // ADBye adblock is INTENTIONALLY NOT consulted here at SYN, unlike
        // malware/firewall above. data->info is an IP-keyed DNS-LRU hostname
        // (set by ip_lru_find in pd_new_connection) that may mismatch a
        // shared CDN IP's real SNI; capture_vpn.c drops the outbound SYN
        // synchronously on to_block right after zdtun_lookup, so a wrong
        // SYN-time block would be final before the real SNI is observed —
        // and check_adblock_sni_rules is the only @@-exemption path
        // (gated by !data->to_block), so a consult + block at SYN cannot
        // be corrected by the post-parse gate. recompute_conn_block_cb does
        // reset to_block in housekeeping, but blacklisted_domain has no
        // adblock-side clear path (only the malware whitelist-reload
        // handler clears it), so for an adblock-flagged connection the
        // reset preserves to_block=true. Net effect: a stale-LRU block
        // violates constraint #2 Resource Protection — adblock stays
        // post-parse-only (the surviving consults are process_ndpi_data's
        // TLS/HTTP host_server_name branches).
    }
}
